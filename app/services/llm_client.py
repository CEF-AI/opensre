...
class DdcDragonLLMClient(OpenAILLMClient):
    ...
    (prompt-based — it embeds the JSON schema and parses the text reply, so no
    special API support is needed) and ``bind_tools``; only the transport is
    overridden to speak the ``{model, input}`` -> ``{output:{text}}`` envelope.
    """
    ...
    def invoke(self, prompt_or_messages: Any) -> LLMResponse:
        ...
        for attempt in range(_RETRY_MAX_ATTEMPTS):
            try:
                ...
                break
            except Exception as err:  # noqa: BLE001 - retried then re-raised as RuntimeError
                last_err = err
