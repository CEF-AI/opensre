...
Miss records live entirely on the engineer's machine in `~/.opensre/misses.jsonl`. The PostHog event carries only the taxonomy, run id, alert name, and optional `user_id` / `org_id` — never the free-text note or the captured root cause string. To delete everything captured locally, remove the file.
